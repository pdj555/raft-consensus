package io.distributed.consensus.integtest;
...
import io.distributed.consensus.transport.netty.NettyRaftCluster;
import java.time.Duration;
import java.util.Set;
import org.junit.jupiter.api.Test;

final class ThreeNodeElectionIT {

    @Test
    void electsLeaderOnLocalhost() throws Exception {
        ...
        try (NettyRaftCluster raftCluster = NettyRaftCluster.create()) {
            raftCluster.addNode(n1, cluster, noopStateMachine(), fastElectionConfig());
            raftCluster.addNode(n2, cluster, noopStateMachine(), fastElectionConfig());
            raftCluster.addNode(n3, cluster, noopStateMachine(), fastElectionConfig());
            raftCluster.startAll();

            final DefaultRaftNode leader = awaitLeader(raftCluster, Duration.ofSeconds(10));
